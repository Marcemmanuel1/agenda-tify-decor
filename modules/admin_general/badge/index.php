...
function isInAllowedLocation($userLat, $userLon) {
    $distance = calculateDistance($userLat, $userLon, COMPANY_LATITUDE, COMPANY_LONGITUDE);
...
<body>
  <div class="container">
    ...
    <div class="section-nav">
      <button class="nav-btn active" id="presenceBtn">
    ...
    <!-- Section Présence -->
    <div class="badge-section active" id="presenceSection">
    ...
    <!-- Section Pause -->
    <div class="badge-section" id="pauseSection">
      <div class="badge-container">
        <div class="badge-card" id="pause-start-card">
          <div class="ripple-effect">
            <div class="ripple"></div>
            <div class="ripple"></div>
            <div class="ripple"></div>
          ...
          <h3>Début Pause</h3>
          <div class="badge-time" id="pause-start-time"></div>
          <div class="status-badge" id="pause-start-status">
            <i class="fas fa-check"></i>
          </div>
        </div>
        
        <div class="badge-card" id="pause-end-card">
          <div class="ripple-effect">
            <div class="ripple"></div>
            <div class="ripple"></div>
            <div class="ripple"></div>
          ...
          <h3>Fin Pause</h3>
          <div class="badge-time" id="pause-end-time"></div>
          <div class="status-badge" id="pause-end-status">
            <i class="fas fa-check"></i>
          </div>
        </div>
      </div>
      ...
      <div class="pause-history">
        <h3>Historique des Pauses Aujourd'hui</h3>
        <div id="pause-history-list">
          <!-- Les pauses seront chargées ici dynamiquement -->
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Éléments de pause
      const pauseStartCard = document.getElementById('pause-start-card');
      const pauseEndCard = document.getElementById('pause-end-card');
      const pauseStartTime = document.getElementById('pause-start-time');
      const pauseEndTime = document.getElementById('pause-end-time');
      const pauseStartStatus = document.getElementById('pause-start-status');
      const pauseEndStatus = document.getElementById('pause-end-status');
      ...
      presenceBtn.addEventListener('click', function() {
        ...
        presenceSection.classList.add('active');
        pauseSection.classList.remove('active');
      });
      
      pauseBtn.addEventListener('click', function() {
        pauseBtn.classList.add('active');
        presenceBtn.classList.remove('active');
        pauseSection.classList.add('active');
        presenceSection.classList.remove('active');
        loadPauseHistory();
      });
      ...
      // Gestion du badgeage de début de pause
      pauseStartCard.addEventListener('click', function() {
        if (!arrivalCard.classList.contains('active')) {
          showNotification('Vous devez d\'abord badger votre arrivée', 'error');
          return;
        }
        if (pauseStartCard.classList.contains('active')) {
          showNotification('Vous avez déjà débuté une pause', 'error');
          return;
        }
        badgePauseStart();
      });
      
      // Gestion du badgeage de fin de pause
      pauseEndCard.addEventListener('click', function() {
        if (!pauseStartCard.classList.contains('active')) {
          showNotification('Vous devez d\'abord débuter une pause', 'error');
          return;
        }
        if (pauseEndCard.classList.contains('active')) {
          showNotification('Vous avez déjà terminé cette pause', 'error');
          return;
        }
        badgePauseEnd();
      });
      ...
      function loadPauseHistory() {
        ...
        .then(data => {
          if (data.success) {
            displayPauseHistory(data.pauses);
            updateCurrentPauseState(data.current_pause);
          }
        })
        .catch(error => {
          console.error('Erreur:', error);
      ...
      function displayPauseHistory(pauses) {
        if (pauses.length === 0) {
          pauseHistoryList.innerHTML = '<div class="pause-item">Aucune pause aujourd\'hui</div>';
          return;
        }
        
        pauseHistoryList.innerHTML = pauses.map(pause => `
          <div class="pause-item">
            <div class="pause-time">${pause.start_time} - ${pause.end_time || 'En cours...'}</div>
            <div class="pause-duration">${pause.duration || ''}</div>
          </div>
        `).join('');
      }
      
      function updateCurrentPauseState(currentPause) {
        if (currentPause && currentPause.start_time && !currentPause.end_time) {
          // Une pause est en cours
          enablePauseStartState(currentPause.start_time);
          pauseEndCard.classList.remove('disabled');
        } else {
          // Aucune pause en cours
          pauseStartCard.classList.remove('active');
          pauseStartStatus.classList.remove('active');
          pauseEndCard.classList.add('disabled');
          pauseEndCard.classList.remove('active');
          pauseEndStatus.classList.remove('active');
        }
      }
      ...
      function badgePauseStart() {
        ...
        .then(data => {
          if (data.success) {
            enablePauseStartState(data.time);
            showNotification(data.message, 'success');
            loadPauseHistory();
          } else {
            showNotification(data.message, 'error');
      ...
      function enablePauseStartState(time) {
        pauseStartTime.textContent = `à ${time}`;
        pauseStartStatus.classList.add('active');
        pauseStartCard.classList.add('active');
        pauseEndCard.classList.remove('disabled');
      }
      
      function enablePauseEndState(time) {
        pauseEndTime.textContent = `à ${time}`;
        pauseEndStatus.classList.add('active');
        pauseEndCard.classList.add('active');
        pauseStartCard.classList.remove('active');
        pauseStartStatus.classList.remove('active');
        ...
        setTimeout(() => {
          ...
          pauseEndCard.classList.remove('active');
          pauseEndStatus.classList.remove('active');
          pauseStartTime.textContent = '';
          pauseEndTime.textContent = '';
          pauseEndCard.classList.add('disabled');
        }, 3000);
      }
